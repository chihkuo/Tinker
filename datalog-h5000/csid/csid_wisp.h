...
#define CSID_C_MULTI_WISP_CNT_TYPE_UI  _CFG_WISP(0x5010)   //T=u8, Connect Type
#define CSID_C_MULTI_WISP_HOSTNAME     _CFG_WISP(0x5020)   //T=str, Client's Host Name
#define CSID_C_MULTI_WISP_IF_IP        _CFG_WISP(0x5030)   //T=ipv4, static interface ip
#define CSID_C_MULTI_WISP_IF_NM        _CFG_WISP(0x5040)   //T=ipv4, static interface netmask
#define CSID_C_MULTI_WISP_IF_GW        _CFG_WISP(0x5050)   //T=ipv4, static interface gateway
#define CSID_C_MULTI_WISP_IF_PRIDNS    _CFG_WISP(0x5060)   //T=ipv4, static interface Primary DNS
#define CSID_C_MULTI_WISP_IF_SECDNS    _CFG_WISP(0x5070)   //T=ipv4, static interface Second DNS
#define CSID_C_MULTI_WISP_MTU          _CFG_WISP(0x5080)   //T=u32, MTU
#define CSID_C_MULTI_WISP_CNT_TYPE     _CFG_WISP(0x5090)   //T=u32, Connect Type
#define CSID_C_MULTI_WISP_CHGTTL       _CFG_WISP(0x50a0)   //T=u8, Enable/Disable , Change the TTL value
#define CSID_C_MULTI_WISP_AUTOBKP      _CFG_WISP(0x50b0)   //T=u8, Enable/Disable , Auto Backup
#define CSID_C_MULTI_WISP_IDLETIME     _CFG_WISP(0x50c0)   //T=u16, D=600, max idle time
#define CSID_C_MULTI_WISP_NAT_DISABLE  _CFG_WISP(0x50d0)   //T=u32, Dynamic IP wantype NAT disable

#define CSID_C_WISP_SCAN_BAND        _CFG_WISP(0x50e0)   //T=u8, Scan Band
...
#define CSID_S_MULTI_WISP_PID          _STA_WISP(0x5010)   //T=u16, The PID of DHCP Client
#define CSID_S_MULTI_WISP_IF_IP        _STA_WISP(0x5020)   //T=ipv4, dynamic interface ip
#define CSID_S_MULTI_WISP_IF_NM        _STA_WISP(0x5030)   //T=ipv4, dynamic interface netmask
#define CSID_S_MULTI_WISP_IF_GW        _STA_WISP(0x5040)   //T=ipv4, dynamic interface gateway
#define CSID_S_MULTI_WISP_IF_PRIDNS    _STA_WISP(0x5050)   //T=ipv4, dynamic interface Primary DNS
#define CSID_S_MULTI_WISP_IF_SECDNS    _STA_WISP(0x5060)   //T=ipv4, dynamic interface Second DNS
#define CSID_S_MULTI_WISP_CNT_STATUS   _STA_WISP(0x5070)   //T=u32, connect status
#define CSID_S_MULTI_WISP_LEASE_TIME   _STA_WISP(0x5080)   //T=u32, uptime + leasetime
#define CSID_S_MULTI_WISP_CNT_TIME     _STA_WISP(0x5090)   //T=u32, connect time
#define CSID_S_MULTI_WISP_PKTIN_NUM    _STA_WISP(0x50a0)   //T=u32, number of packet input
#define CSID_S_MULTI_WISP_PKTOUT_NUM   _STA_WISP(0x50b0)   //T=u32, number of packet output
