...
#define LOCAL_WANTYPE_ADSL_DHCP			0x30
#define LOCAL_WANTYPE_ADSL_FIXED 		0x31
#define LOCAL_WANTYPE_PPPOE             0x40
#define LOCAL_WANTYPE_PPPOE_MULTI       0x41
#define LOCAL_WANTYPE_PPPOE_RUSSIAN     0x42
#define LOCAL_WANTYPE_PPPOE_ADSL        0x43
...
#define CSID_C_LOCAL_REMOTE_SSH_IP          _CFG_LOCAL(0x007C) //T=ipv4, the remote IP for SSH
#define CSID_C_LOCAL_REMOTE_SSH_NM          _CFG_LOCAL(0x007D) //T=u16, the remote netmask for SSH
#define CSID_C_LOCAL_SSH_REMOTE_ENABLE      _CFG_LOCAL(0x007E) //T=u8, D=0, 1:yes 0:no, enable ssh remote
...
/* Have SFP*/
#define CSID_C_LOCAL_HAVE_SFP   _CFG_LOCAL(0x0143) //T=u8, have SFP? 0:no, 1:yes
#define CSID_C_LOCAL_SFP_INDEX  _CFG_LOCAL(0x0144) //T=u32, SFP CSID index
...
#define CSID_C_LOCAL_WAN4_INDEX	_CFG_LOCAL(0x0603) //T=u8, D=0, record WAN index picked to fourth WAN
...
/* Guest account */
#define CSID_C_LOCAL_GUEST3_ACCOUNT             _CFG_LOCAL(0x0910) //T=str, D="user"
#define CSID_C_LOCAL_GUEST3_PASSWORD            _CFG_LOCAL(0x0920) //T=str, D="user"

/*IPV6 MLD*/
#define CSID_C_LOCAL_WAN_MLD_VERSION		_CFG_LOCAL(0x6010) //T=u32, define local wan MLD version

/*physical interface & internet setup CSID for UI*/
#define CSID_C_LOCAL_WAN_OP_MODE_UI     _CFG_LOCAL(0x1000) //T=u32, D=0, 0:Disable, 1:Always on, 2:Failover
#define CSID_C_LOCAL_WAN_WANTYPE_UI     _CFG_LOCAL(0x1010) //T=u32, D=-1, define wantype value for UI
#define CSID_C_LOCAL_WAN_VLAN_ENABLE_UI _CFG_LOCAL(0x1020) //T=u32, D=0, 0:Disable, 1:Enable
#define CSID_C_LOCAL_WAN_VLAN_VID_UI    _CFG_LOCAL(0x1030) //T=u32, D=0, WAN Tag
#define CSID_C_LOCAL_WAN_WANSET_ALTER_ADDR _CFG_LOCAL(0x1040) //T=str, D=0, WANSET Alter ADDR
#define CSID_C_LOCAL_WAN_ENABLE_ADDR    _CFG_LOCAL(0x1050) //T=str, D=0, WAN Enable ADDR
#define CSID_C_LOCAL_WAN_FAILOVER_ADDR _CFG_LOCAL(0x1060) //T=str, D=0, WAN Failover ADDR

/*ICMP check Item*/
#define CSID_C_LOCAL_ICMPSEND_ENABLE			_CFG_LOCAL(0x1100)	//T=u8, D=0, 0:disable, 1:enable
#define CSID_C_LOCAL_ICMPSEND_IP				_CFG_LOCAL(0x1110)  //T=ipv4,D={0.0.0.0}, ICMPSEND destination IP
#define CSID_C_LOCAL_ICMPSEND_TIME_INTERVAL		_CFG_LOCAL(0x1120)	//T=u32, D=900, ICMP send time interval
#define CSID_C_LOCAL_ICMPSEND_IFNAME			_CFG_LOCAL(0x1130)	//T=str, icmp send interface
#define CSID_C_LOCAL_ICMPSEND_DEBUG				_CFG_LOCAL(0x1140)	//T=u8, D=0, 0:disable, 1:enable

/* Device Link Item */
#define CSID_C_LOCAL_DEVICE_LINK_ENABLE         _CFG_LOCAL(0x1150)  //T=u8, D=0, 0:disable, 1:enable
#define CSID_C_LOCAL_DEVICE_LINK_POLL_TIME      _CFG_LOCAL(0x1151)  //T=u32, D=30 
#define CSID_C_LOCAL_DEVICE_LINK_PORT           _CFG_LOCAL(0x1152)  //T=u32, D=5555

/* Power Over Ethernet (Reserved: 0x2000 ~ 0x2FFF) */
#define CSID_C_LOCAL_POE_POWER_BUDGET                       _CFG_LOCAL(0x2000)  //T=u32, D=1, PoE Power Budget, 0: Default (Reserved), 1: 120 Watts, 2: 60 Watts, 3: Manual
#define CSID_C_LOCAL_POE_POWER_BUDGET_MANUAL_THRESHOLD      _CFG_LOCAL(0x2001)  //T=u32, PoE Power Budget Manual Threshold, Value Range: 4 ~ 120 Watts

#define CSID_C_LOCAL_POE_PORT_POWER_LIMIT                   _CFG_LOCAL(0x2100)  //T=u32, D=0, PoE Port Power Limit, 0: Auto, 1: 802.3af (4W), 2: 802.af (7W), 3: 802.3af(15.4W), 4: 802.3at(30W), 5: Manual
#define CSID_C_LOCAL_POE_PORT_POWER_LIMIT_MANUAL_THRESHOLD  _CFG_LOCAL(0x2140)  //T=u32, PoE Port Power Limit Manual Threshold, Value Range: 1 ~ 30 Watts
#define CSID_C_LOCAL_POE_PORT_PRIORITY                      _CFG_LOCAL(0x2180)  //T=u32, D=1, PoE Port Priority, Value Range: 1 ~ 3, 1 is highest
#define CSID_C_LOCAL_POE_PORT_PD_PING_CHECK_ENABLE          _CFG_LOCAL(0x21C0)  //T=u32, D=0, PoE Port PD Ping Check Enable, 0: Disable, 1: Enable
#define CSID_C_LOCAL_POE_PORT_PD_PING_CHECK_IP              _CFG_LOCAL(0x2200)  //T=ipv4, PoE Port PD Ping Check IP Address
#define CSID_C_LOCAL_POE_PORT_PD_PING_CHECK_TIMEOUT         _CFG_LOCAL(0x2240)  //T=u32, D=30, PoE Port PD Ping Check Timeout, Value Range: 10 ~ 300 seconds
#define CSID_C_LOCAL_POE_PORT_PD_NO_RESPONSE_ACTION         _CFG_LOCAL(0x2280)  //T=u32, D=0, PoE Port PD No Response Action, 0: No Action, 1: Power Off/On 
#define CSID_C_LOCAL_POE_PORT_PD_POWER_OVERLOAD_ACTION      _CFG_LOCAL(0x22C0)  //T=u32, D=0, PoE Port PD Power Overload Action, 0: No Action, 1: Long Time Power Off/On
#define CSID_C_LOCAL_POE_PORT_RULE_SCHE_NO                  _CFG_LOCAL(0x2300)  //T=u32, D=0, PoE Port Schedule number of using rule
#define CSID_C_LOCAL_POE_PORT_POWER_OUTPUT_DISABLE          _CFG_LOCAL(0x2340)  //T=u32, D=0, PoE Port Power Output Disable
...
/* Password change altered*/
#define CSID_S_LOCAL_PASSWORD_ALTERED   _STA_LOCAL(0x004b)	//T=u8, D=0, password change alter

/*Used to check if LAN IP is valid by change_dhcpd_ip_pool.c*/
#define CSID_S_LOCAL_LANIP_CHECK		_STA_LOCAL(0x004c) //T=u8, D=0, 0: Invalid LAN IP, 1: Valid LAN IP
...
/* Record traffic for lan*/
#define CSID_S_LOCAL_LAN_TX_TRAFFIC				_STA_LOCAL(0x00B0)	//T=u32, record br+ tx traffic for LAN
#define CSID_S_LOCAL_LAN_RX_TRAFFIC				_STA_LOCAL(0x00C0)	//T=u32, record br+ rx traffic for LAN

/* Record traffic accum for do something*/
#define CSID_S_LOCAL_MULTIWAN_ACCUM				_STA_LOCAL(0x00D0)	//T=u32, D=0, record max fail accum.
#define CSID_S_LOCAL_LOADCHECK_ACCUM			_STA_LOCAL(0x00E0)	//T=u32, D=0, multiwanchk2 load check accum
#define CSID_S_LOCAL_PINGKEEP_ACCUM				_STA_LOCAL(0x00F0)	//T=u32, D=0, multiwanchk2 ping keep accum
...
#define CSID_S_LOCAL_WIZARD_WANEXEC_ALTER           _STA_LOCAL(0x309a)    //T=u32, D=0, Test Wan Execution Status alter for IO-Data Wizard
...
#define CSID_S_LOCAL_WAN_FAILCOUNTS                _STA_LOCAL(0x5180) //T=u32, D=0, wan connect fail times count

/* device link ask status */
#define CSID_S_LOCAL_DEVICE_LINK_STATUS            _STA_LOCAL(0x5190)  //T=u8, D=0, 0:getfull  1:getupdate
#define CSID_S_LOCAL_DEVICE_LINK_STATUS_2          _STA_LOCAL(0x5210)  //T=u8, D=0, 0:getfull  1:getupdate

/* Handle_routing status */
#define CSID_S_LOCAL_ROUTE_STATUS                     _STA_LOCAL(0x5220)  //T=u8, D=0, 1:Running
#define CSID_S_LOCAL_MULTIWAN_ROUTE_STATUS            _STA_LOCAL(0x5230)  //T=u8, D=0, 1:Running

/* Power Over Ethernet Port Status (Reserved: 0x6000 ~ 0x6FFF) */
#define CSID_S_LOCAL_POE_ALTERED                   _STA_LOCAL(0x6000)  //T=u32, poe related ui altered

#define CSID_S_LOCAL_POE_PORT_POWER_OUTPUT_STATUS  _STA_LOCAL(0x6100)  //T=u32, D=0, PoE Port Power Output Status, 0: Off, 1: On
#define CSID_S_LOCAL_POE_PORT_PD_CLASS             _STA_LOCAL(0x6140)  //T=u32, D=0, PoE Port PD Class, 0: N/A (Power Off), 1: Class 1, 2: Class 2, 3: Class 3, 4: Class 4, 5: Class Error, 99: Class 0
#define CSID_S_LOCAL_POE_PORT_VOLTAGE              _STA_LOCAL(0x6180)  //T=u32, D=0, PoE Port Voltage, Unit: dV
#define CSID_S_LOCAL_POE_PORT_CURRENT              _STA_LOCAL(0x61C0)  //T=u32, D=0, PoE Port Current, Unit: mA
#define CSID_S_LOCAL_POE_PORT_CONSUMPTION          _STA_LOCAL(0x6200)  //T=u32, D=0, PoE Port Consumption, Unit: mW
#define CSID_S_LOCAL_POE_PORT_NO_RESPONSE_STATUS   _STA_LOCAL(0x6240)  //T=u32, D=0, PoE Port No Reponse Status, 0: Normal, 1: No Response
#define CSID_S_LOCAL_POE_PORT_STATUS               _STA_LOCAL(0x6280)  //T=u32, D=0, PoE Port Actual Status
#define CSID_S_LOCAL_POE_PORT_ENABLE_STATUS        _STA_LOCAL(0x62C0)  //T=u32, D=0, PoE Port Channel Enable Status
